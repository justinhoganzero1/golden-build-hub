...
/** Huge mid-ad proof banner: the whole ad was generated by Oracle Lunar. */
const MadeInBanner: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const s = spring({ frame, fps, config: { damping: 20, stiffness: 150 } });
  const out = interpolate(frame, [S(3.4), S(4.0)], [1, 0], {
  ...
  return (
    ...
    >
      <div
        style={{
          transform: `scale(${pulse * interpolate(s, [0, 1], [0.86, 1])})`,
          padding: "42px 70px",
          borderRadius: 26,
          border: `3px solid ${GOLD}`,
          background: "rgba(4,3,8,0.86)",
          boxShadow: `0 0 140px rgba(245,201,122,0.5), 0 40px 100px rgba(0,0,0,0.9)`,
          textAlign: "center",
          maxWidth: 1500,
        }}
      >
        ...
        <div
          style={{
            ...
            lineHeight: 1.05,
            color: "#fff",
            textShadow: `0 0 70px rgba(245,201,122,0.55)`,
          }}
        >
          This entire ad was
          <br />
          <span style={{ color: GOLD_HOT }}>generated by Oracle Lunar.</span>
        </div>
      </div>
    </AbsoluteFill>
  );
