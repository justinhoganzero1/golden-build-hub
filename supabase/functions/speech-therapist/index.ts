...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const __rl = await enforceRateLimit(req, auth.user, "speech-therapist");
    if (__rl) return __rl;
    ...
    // Bill the user's own wallet only after the rewrite actually succeeded.
    try {
      await chargeAI(auth.user.id, "speech-therapist", PROVIDER_RATES.lovable_ai_gemini_flash_per_call, {
        provider: "lovable-ai",
        model: "google/gemini-2.5-flash-lite",
        persona,
        qaApplied,
      ...
      console.error("speech-therapist billing error:", billErr);
    }
