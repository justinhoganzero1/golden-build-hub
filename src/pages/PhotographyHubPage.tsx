    ...
    try {
      ...
      if (user) {
        saveMedia.mutate({
          ...
          url: finalUrl,
          source_page: "photography-hub",
          metadata: { filter: selectedFilter, mode, prompt, resolution: "4k" },
        });
      }
    } catch (e) {
      console.error(e);
            ...
            <div className="aspect-square bg-card border border-primary/30 rounded-xl overflow-hidden relative">
              ...
              <div className="absolute top-2 right-2 flex gap-2">
                ...
                <button onClick={async () => {
                  try {
                    await downloadFileFromUrl(generatedImage, `oracle-lunar-4k-photo-${Date.now()}`);
          ...
          onSave={(newUrl) => {
            ...
            if (user) {
              saveMedia.mutate({
                ...
                url: newUrl,
                source_page: "photography-hub",
                metadata: { editedInStudio: true, basePrompt: prompt, resolution: "4k" },
              });
            }
          }}
        />
