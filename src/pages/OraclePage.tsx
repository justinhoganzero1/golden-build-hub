...
const OraclePage = () => {
  ...
  const handleSetupReply = useCallback(async (rawText: string): Promise<boolean> => {
    ...
    // Triggered by phrases like "build me a personal trainer website with voice instructions
    // and food planning + ordering", "make a working e-commerce site", "create a finished
    // app that does X". Runs the multi-stage architect→backend→frontend→flesh→smoke pipeline
    // entirely in the background and saves the finished single-file HTML site to the
    // user's Library (source_page: "app-builder", media_type: "app"). The user can keep
    // chatting normally while it builds.
    const buildMatch = isIntroTrigger ? null : text.match(
      /(?:build|make|create|generate|design|develop|code|put together|spin up|whip up|finish)(?:\s+(?:me|us))?\s+(?:a|an|the|my|our|some)?\s*(?:complete|full|finished|working|whole|entire|production|real|live)?\s*(?:personal\s+\w+\s+)?(?:web ?site|website|web\s*app|webapp|app|application|landing\s*page|store|shop|portal|dashboard|platform|saas|tool|page|site)\b([^]*)/i
    );
    if (buildMatch) {
